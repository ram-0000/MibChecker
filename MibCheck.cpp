...
void MibCheck::_check(OverwriteAlgorithm Algo, const QString & File) throw (ParserException)
{
	// record filename
	m_mib_filename = File;
...
void MibCheck::_def(const QString & Name) throw (ParserException)
{
	// create filename
	QFile file(Name);
...
void MibCheck::_html(const QString & Name) throw (ParserException)
{
	// create filename
	QFile file(Name);
	if(file.open(QIODevice::WriteOnly) == false)
...
void MibCheck::_check_overwrite(OverwriteAlgorithm Algo,
										  ...
										  const QString & output_mib) const throw (ParserException)
{
